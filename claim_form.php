...
/*
 * Approvers can view any staff member's monthly form.
 */
if ($role === 'approver') {
...
/*
 * Get approved OT requests for the 21st-to-20th claim period.
 */
$stmt = $pdo->prepare(
    'SELECT
        id,
...
/*
 * Separate Sunday OT from Monday-Saturday OT.
 *
 * Sunday always uses the MEMO form regardless of staff category.
 */
$sundayRequests = [];
$weekdayRequests = [];
...
<div class="dash-header no-print">
    ...
    <div style="display:flex;gap:.5rem;flex-wrap:wrap;">
        ...
        <a
            href="calendar.php?y=<?= (int)substr($period['end'], 0, 4) ?>&m=<?= (int)substr($period['end'], 5, 2) ?><?= $role === 'approver' ? '&staff_id=' . (int)$staffId : '' ?>"
            class="btn-secondary"
        >
            Back to Calendar
        </a>
            ...
            <div>
                ...
                >
                    ...
                    <?php foreach ($allStaff as $person): ?>
                        ...
                        <option
                            value="<?= (int)$person['id'] ?>"
                            <?= (int)$person['id'] === $staffId ? 'selected' : '' ?>
                        >
                            <?= h($person['name']) ?>
                            (<?= h($person['staff_no']) ?>)
                        </option>
...
<?php if ($category === null): ?>
    ...
    <div class="form-error no-print">
        This staff member's category has not been confirmed by HR yet.
    </div>
    ...
    <div class="claim-document-inner">
        ...
        <?php if ($category === 'ae_above'): ?>
            ...
            <div class="claim-title-center">
                ...
                <h2>
                    TEMPORARY MEAL ALLOWANCE CLAIM FORM
                </h2>
            ...
            <table class="claim-table">
                ...
                <?php if (!$categoryRequests): ?>
                    ...
                    <tr>
                        <td colspan="6" class="claim-empty">
                            No approved Monday–Saturday OT records for this period.
                        </td>
                    </tr>
                ...
                <?php else: ?>
                    ...
                    <?php foreach ($categoryRequests as $r): ?>
                        ...
                        <tr>
                            ...
                            <td>
                                <?= h(date('d/m/Y', strtotime($r['ot_date']))) ?>
                            </td>

                            <td>
                                <?= h(substr($r['start_time'], 0, 5)) ?>
                            </td>

                            <td>
                                <?= h(substr($r['end_time'], 0, 5)) ?>
                            </td>

                            <td>
                                <?= h(number_format((float)$r['total_hours'], 2)) ?>
                            </td>
            ...
            <div class="claim-notes">
                ...
                <ol>
                    ...
                    <li>
                        For salary RM 2000 above (below assistant manager)
                    </li>

                    <li>
                        Flat rate of RM 20 per day
                    </li>

                    <li>
                        Min 2 hrs of extra working hours
                    </li>

                    <li>
                        Must get HOD's approval and submit to HR before end each week on weekly basis
                    </li>
        ...
        <?php else: ?>
            ...
            <div class="memo-department">
                (<?= h($staff['department'] ?: 'DEPARTMENT NAME') ?>)
            </div>
            ...
            <table class="memo-meta">
                ...
                <tr>
                    <th>To</th>
                    <td>Human Resources Department / Payroll department</td>
                </tr>
                ...
                <tr>
                    ...
                    <td>
                        <strong>
                            CLAIM OVERTIME 08:00 PM to 10:00 PM
                        </strong>
                    </td>
                </tr>
            ...
            <p>
                With reference to above, please be informed that below workers will be work on 08:00 pm to 10:00 pm.
            </p>

            <p>
                Kindly make sure a necessary arrangement for this:
            </p>
            ...
            <table class="claim-table memo-table">
                ...
                <?php if (!$categoryRequests): ?>
                    ...
                    <tr>
                        <td colspan="5" class="claim-empty">
                            No approved Monday–Saturday OT records for this period.
                        </td>
                    </tr>
...
<?php if (!$requests): ?>
    ...
    <div class="empty-state no-print">
        There are no approved OT records for this claim period.
    </div>
